...
local IngredientsView = require(ModuleIndex.InventoryIngredientsView)
local DrinksView = require(ModuleIndex.InventoryDrinksView)
...
local TAB_NAMES = {"Ingredients", "Drinks"}
...
function InventoryPage:render()
	...
	return createElement(PageWrapper, {isOpen = (currentPage == "Inventory")}, {
		...
		Window = createElement(Window, {title = "Inventory", Position = UDim2.fromScale(0.5, 0.5), Size = UDim2.fromScale(0.6, 0.7), AnchorPoint = Vector2.new(0.5, 0.5), onExit = onExit}, {
			IngredientsView = createElement(IngredientsView, {Visible = currentView == "Ingredients"}),
			DrinksView = createElement(DrinksView, {Visible = currentView == "Drinks"})
		})
	})
end
